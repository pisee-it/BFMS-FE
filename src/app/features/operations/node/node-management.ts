...
export class NodeManagementComponent implements OnInit {
  ...
  saveNode() {
    ...
    if (this.nodeForm.valid && route) {
      ...
      this.nodeService.createNode(route.id, nodeData).subscribe({
        next: () => {
          this.messageService.add({ severity: 'success', summary: 'Thành công', detail: 'Đã tạo nốt xe mới' });
          this.nodeDialog.set(false);
          this.loadNodes(route.id);
        },
        error: (error) => {
          const detail = error.error?.message || 'Không thể tạo nốt xe';
