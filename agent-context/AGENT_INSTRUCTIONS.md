...
Bạn là AI Agent làm việc trên dự án **BFMS (Bus Finance Management System)**.  
Đây là tài liệu điều phối trung tâm. Đọc đủ, đọc hết, không bỏ qua bước nào.
...
3. **Trình bày kế hoạch thực thi** ngắn gọn (bullet points, không code):
   - Chia kế hoạch thành các giai đoạn (Phases) tương ứng với từng **Subtask** trong Jira.
   - Mỗi giai đoạn phải đảm bảo tính độc lập để có thể **commit riêng biệt** (Atomic Commits).
   - Xác định rõ file nào sẽ tạo mới/sửa đổi trong từng giai đoạn.
   - Có điểm nào cần confirm với owner không?
4. **Dừng lại và chờ xác nhận** từ người dùng trước khi viết bất kỳ dòng code nào.
...
Sau khi hoàn thành **mỗi giai đoạn (Phase/Subtask)**:

1. Tạm dừng thực thi.
2. Cập nhật `agent-context/PROJECT_MEMORY.md`: Ghi lại subtask vừa hoàn thành, các quyết định quan trọng. Tuân thủ quy tắc bóc tách theo subtask.
3. Cập nhật (hoặc tạo nháp) file report tại `agent-context/angular-frontend/execution_reports/sprint-fe01/JiraID.md`: Bổ sung dần nội dung của subtask vừa làm vào mục tương ứng.
4. Gợi ý lệnh commit theo chuẩn: `<type>(<scope>): [Mã subtask] <mô tả ngắn>` (ví dụ: `feat(auth): [PRJBFMS-239] Khởi tạo cấu hình Signals`).
   - **Lưu ý**: KHÔNG bao gồm bất kỳ file nào trong thư mục `agent-context` vào lệnh commit (thư mục này chỉ dùng để AI lưu trữ bối cảnh tại local).
5. Thông báo cho người dùng và chờ chỉ thị tiếp theo.

> ❌ NGHIÊM CẤM: Chạy liên tiếp nhiều subtask mà không dừng lại cập nhật và thông báo.
...
Khi đã hoàn tất toàn bộ task và đã đẩy code (push) lên server, bạn PHẢI:

1. Gợi ý **Title** cho Pull Request theo chuẩn: `feat/fix/chore...: [Mã Task] Tên task`.
2. Viết **Description** tóm tắt các subtask đã hoàn thành và các lưu ý quan trọng để người dùng copy vào PR trên GitHub/GitLab.
3. Gợi ý **Merge Commit Message** & **Extended Description** để sử dụng khi squash/merge PR.
